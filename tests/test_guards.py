...
_LEAK = [
    ...
    ("범한", "실명 거래처"),
    ("LS오토", "실명 거래처"),
    # ⚠️ 2026-09-11 **개인 이름은 빼지 않기로 했다**(기쁨 결정). 09-05 에는
    #    71건을 치환했으나 오늘 기준이 바뀌었다 — 대신 **회사명**을 막는다.
    #    회사명은 소스에는 일부러 남기므로(CLAUDE.md 「밖으로 나갈 때」) 여기서
    #    훑지 않고, **나가는 문서**에서 `proposal.self_check()` 가 본다.
]
...
# ⚠️ 표본은 **검사 목록과 같은 형태**여야 한다. 내 PC 경로 패턴은 소스에 적히는
#    꼴(백슬래시 둘)이라, 표본에 하나만 쓰면 **검사가 자기를 못 잡는다**
#    (2026-09-11 에 실제로 FAIL 이 났다 — 그게 자가검증이 하는 일이다).
_probe = ("wiki/decisions/" + "어떤노트.md "
          + "C:" + chr(92) + chr(92) + "Users")
check(sum(1 for n, _ in _LEAK if n in _probe) >= 2,
      "  · 검사 목록이 볼트 경로와 내 PC 경로를 둘 다 잡는다 (자가검증)")
