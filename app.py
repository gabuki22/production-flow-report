...
st.set_page_config(page_title=C.APP_NAME, page_icon="📊",
...
cards = [
    ...
    ("📊 대시보드", "퍼널 · 실험 · 유효 원가를 봅니다.", "pages/2_대시보드.py", "반복해서 봄"),
    ("📄 리포트", "8장 문서와 PDF를 만듭니다.", "pages/3_리포트.py", "만들어 공유"),
    ("🗂️ 아카이브", "지난 실행을 찾고 비교합니다.", "pages/4_아카이브.py", "가끔"),
    ("📈 납기 전망", "밀린 수주가 언제 풀리는지 봅니다.", "pages/5_납기전망.py", "주 1회"),
]
# ★ 칸 수를 카드 수에서 만든다. 전에는 st.columns(4) 를 박아 두고 zip 했는데,
#   카드를 다섯으로 늘리자 **다섯 번째가 조용히 사라졌다** — 에러도 안 난다.
#   목록과 레이아웃 둘 다 고쳐야 하는 구조는 언젠가 한쪽만 고친다.
for col, (t, d, p, when) in zip(st.columns(len(cards)), cards):
    with col:
        st.markdown(
